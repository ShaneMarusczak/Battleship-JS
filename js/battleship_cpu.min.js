...
c.removeEventListener("mouseleave",fa);c.removeEventListener("click",T);c.removeEventListener("mouseover",U);c.removeEventListener("mouseleave",V);c.classList.remove("curserPointer")}}}let J,K,w,l,C="hor",W=0,L=0,n="",M=0,x=0,I=!1,X=!1,Y=!1,Z=!1,aa=!1,ba=!1,y=!1,N=!1,O=!1,P=!1,Q=!1,R=!1;const h=[],oa=document.getElementById("gameboard_cpu"),pa=document.getElementById("carrier"),qa=document.getElementById("battleship"),ra=document.getElementById("cruiser"),sa=document.getElementById("submarine"),ta=
document.getElementById("destroyer"),ua=new Audio("sounds/Ship Sunk Sound.mp3"),S=[],va=[pa,qa,ra,sa,ta],m={battleship:4,carrier:5,cruiser:3,destroyer:2,submarine:3},ha="Found You!;You're Sunk!;Down she goes!;Gotcha!;Ha Ha!;I'm the best!;Woo Hoo!;I'm Better!;I'm gonna win!;Easy!;Sink that ship!;Try Harder!;Yawn...;Sunk!;Oh Yeah!;You're Gonna Lose!;Really? Easy..;Yes!!;So Easy!;Child's Play!;:)".split(";"),q=(b,a)=>0<=b&&10>b&&0<=a&&10>a,D=(b,a)=>{ua.play();document.getElementById(a+"Sunk_cpu").classList.remove("hitText");
...
if(0<L)a:for(a=0;10>a;a++)for(b=0;10>b;b++)if(u(a,b)){J=a;K=b;break a}},ja=()=>{for(let b=0;10>b;b++)for(let a=0;10>a;a++)"hor"===C?(document.getElementById("c"+b+a).removeEventListener("mouseover",ca),document.getElementById("c"+b+a).addEventListener("mouseover",ia)):(document.getElementById("c"+b+a).removeEventListener("mouseover",ia),document.getElementById("c"+b+a).addEventListener("mouseover",ca));C="hor"===C?"ver":"hor"},ia=b=>{const a=b.target.id;let c=!0;for(let f=0;f<l;f++)if(!t(Number(a[1])+
f,Number(a[2]))){c=!1;break}if(Number(a[1])<11-l&&c)for(b.target.addEventListener("click",E),b=0;b<l;b++)document.getElementById(a[0]+(Number(a[1])+b)+a[2]).classList.add("blackBackground");else b.target.removeEventListener("click",E)},ca=b=>{const a=b.target.id;let c=!0;for(let f=0;f<l;f++)if(!t(Number(a[1]),Number(a[2])+f)){c=!1;break}if(Number(a[2])<11-l&&c)for(b.target.addEventListener("click",E),b=0;b<l;b++)document.getElementById(a[0]+a[1]+(Number(a[2])+b)).classList.add("blackBackground");
else b.target.removeEventListener("click",E)},fa=()=>{for(let b=0;10>b;b++)for(let a=0;10>a;a++)t(b,a)&&(document.getElementById("c"+b+a).classList.remove("blackBackground"),document.getElementById("c"+b+a).classList.remove("greyBackground"))},T=b=>{if(0===l&&!window.gameStarted){let a;const c=Number(b.target.id[1]),f=Number(b.target.id[2]),[,e]=h[c][f];switch(e){case "carrier":X=!1;break;case "battleship":Y=!1;break;case "cruiser":Z=!1;break;case "submarine":aa=!1;break;case "destroyer":ba=!1}l=
m[e];w=e;document.getElementById(e).classList.remove("hidden");ka.forEach(d=>document.getElementById(d).classList.remove("clicked"));document.getElementById(e).classList.add("clicked");x--;a:for(const d of S)for(const g of d)if(g[0]===c&&g[1]===f){C="hor"===h[g[0]][g[1]][2]?"ver":"hor";a=d;break a}a.forEach(d=>{const g=document.getElementById("c"+d[0]+d[1]);g.removeEventListener("mouseover",U);g.removeEventListener("mouseleave",V);g.removeEventListener("click",T);g.classList.remove("curserPointer");
...
"carrier"!==a||X)?"battleship"!==a||Y?"cruiser"!==a||Z?"submarine"!==a||aa?"destroyer"!==a||ba||(l=m[a],b.target.parentElement.classList.add("clicked")):(l=m[a],b.target.parentElement.classList.add("clicked"),w="submarine"):(l=m[a],b.target.parentElement.classList.add("clicked"),w="cruiser"):(l=m[a],b.target.parentElement.classList.add("clicked")):(l=m[a],b.target.parentElement.classList.add("clicked"))}},U=b=>{if(0===l){[,b]=h[Number(b.target.id[1])][Number(b.target.id[2])];for(let a=0;10>a;a++)for(let c=
0;10>c;c++)r(a,c,b)&&(document.getElementById("c"+a+c).classList.remove("blackBackground"),document.getElementById("c"+a+c).classList.add("greyBackground"))}},V=b=>{[,b]=h[Number(b.target.id[1])][Number(b.target.id[2])];for(let a=0;10>a;a++)for(let c=0;10>c;c++)r(a,c,b)&&(document.getElementById("c"+a+c).classList.add("blackBackground"),document.getElementById("c"+a+c).classList.remove("greyBackground"))},wa=()=>{for(let b=0;10>b;b++)for(let a=0;10>a;a++)document.getElementById("c"+b+a).classList.contains("greyBackground")&&
...
l===m.destroyer&&F(a,c,"destroyer");const f=document.getElementById("c"+a+c);f.addEventListener("click",T);f.addEventListener("mouseover",U);f.addEventListener("mouseleave",V);f.removeEventListener("click",E);f.classList.add("curserPointer");h[a][c][0]=1;b.push([a,c])}l===m.carrier?(X=!0,x++,document.getElementById("carrier").classList.add("hidden")):l===m.battleship?(Y=!0,x++,document.getElementById("battleship").classList.add("hidden")):l===m.cruiser?"cruiser"===w?(Z=!0,x++,document.getElementById("cruiser").classList.add("hidden")):
"submarine"===w&&(aa=!0,x++,document.getElementById("submarine").classList.add("hidden")):l===m.destroyer&&(ba=!0,x++,document.getElementById("destroyer").classList.add("hidden"));5===x&&(y=I=!0,document.getElementById("startGame").classList.remove("notDisplayed"),document.getElementById("instructions").innerHTML="<b>Placed ships can be moved before starting!</b>");l=0;S.push(b)}},r=(b,a,c)=>q(b,a)&&h[b][a][1]===c,za=()=>{if(N&&O&&P&&Q&&R){document.getElementById("losstext").style.display="block";
...
W++},z=(b,a)=>{document.getElementById("c"+b+a).classList.remove("blackBackground");document.getElementById("c"+b+a).classList.add("missBackground");h[b][a][0]=3;M++;W++},ma=()=>{let b,a,c=0;if(4>W||3<M){do{b=window.randomIntFromInterval(0,9);a=window.randomIntFromInterval(0,9);if(10<c){[b,a]=ea(!1);break}c++}while(0!==b%2&&0===a%2||0===b%2&&0!==a%2||[2,3,4,5,7].includes(b)||[2,4,5,6,7].includes(a));0<M?1:0}else[b,a]=ea(!1);J=b;K=a;t(b,a)?z(b,a):A(b,a)?B(b,a):p(b,a)&&ma()},Da=()=>{if(!window.gameOver&&
window.gameStarted)if(y){0<L?Ca():ma();ea(!0);for(var b=0;10>b;b++)for(var a=0;10>a;a++)u(b,a)&&document.getElementById(h[b][a][1]+"Sunk_cpu").classList.add("hitText");var c=a=b=0,f=0,e=0;for(let d=0;10>d;d++)for(let g=0;10>g;g++)!N&&r(d,g,"carrier")&&u(d,g)?b++:!O&&r(d,g,"battleship")&&u(d,g)?a++:!P&&r(d,g,"cruiser")&&u(d,g)?c++:!Q&&r(d,g,"submarine")&&u(d,g)?f++:!R&&r(d,g,"destroyer")&&u(d,g)&&e++;b===m.carrier&&(N=!0,D(5,"carrier"));a===m.battleship&&(O=!0,D(4,"battleship"));c===m.cruiser&&(P=
!0,D(3,"cruiser"));f===m.submarine&&(Q=!0,D(3,"submarine"));e===m.destroyer&&(R=!0,D(2,"destroyer"));za()}else window.modal("Place all ships!",1400)},la=()=>{const b=[];N||b.push(m.carrier);O||b.push(m.battleship);Q||b.push(m.submarine);P||b.push(m.cruiser);R||b.push(m.destroyer);return b},ea=b=>{var a=la(),c=!b;const f=[];for(var e=0;10>e;e++){f.push([]);for(var d=0;10>d;d++)c?f[e].push(0===window.randomIntFromInterval(0,2)?0:window.randomIntFromInterval(0,9)):f[e].push(0)}e=0;for(c=0;c<a.length;c++){for(d=
...
"rotateBackAnimation")};(()=>{window.compMoveWindow=Da;for(let b=0;10>b;b++){h.push([]);for(let a=0;10>a;a++){h[b].push([0,""]);const c=document.createElement("div");oa.appendChild(c);c.id="c"+a+b;const f=50*b+5;c.style.top=50*a+5+"px";c.style.left=f+"px";c.style.backgroundColor="#80aaff";c.addEventListener("mouseover",ca);c.addEventListener("mouseleave",fa)}}va.forEach(b=>b.addEventListener("click",xa));window.addEventListenerById("rotate","mouseover",Ea);window.addEventListenerById("rotate","mouseleave",
Fa);window.addEventListenerById("rotate","click",ja);window.addEventListenerById("startGame","click",na);document.getElementById("compWins").textContent="Computer Wins: "+window.compWinsOnLoad()})()})();
